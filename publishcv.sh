...
do
    inform "Publish CV ${cv}"
	    "/usr/bin/hammer content-view publish --name \"${cv}\" --organization \"${ORG}\" --description \"Build ${BUILD_ID} of Job ${JOB_NAME} on ${JENKINS_URL}\"" || \
    ...
    VER_ID_LIST+=( "$("/usr/bin/hammer content-view info --name \"${cv}\" --organization \"${ORG}\" \
...
inform "Analysing CCVs"
    ...
    do
        ...
        CV_VER_SED+="$("/usr/bin/hammer --csv content-view version list --content-view \"${cv}\" --organization \"${ORG}\"" | awk -F',' -v ver_id="${ver_id}" '
                $1 != "ID" && $1 != ver_id {ids="s/," $1 ",/," ver_id ",/;" ids}
                END {print ids}')"
    done

    # Create an array of composite content view IDs matching the given pattern
    CCV_TMP_IDS=( $("/usr/bin/hammer --csv content-view list --organization \"${ORG}\" --search \"${CCV_NAME_PATTERN}\"" | awk -F, '$1 ~ /^[0-9]+$/ {print $1}') )
    ...
    do
	cv_tmp_ver_ids=$("/usr/bin/hammer --output yaml content-view info --id ${ccv_id} --organization \"${ORG}\"" \
	    | awk -F': *' '
...
then # there is at least one CCV using the given CVs
    ...
    do
            ...
            inform "Update component IDs of CCV ${ccv_id}"
                "/usr/bin/hammer content-view update --id ${ccv_id} --component-ids \"${cv_used_ver_ids}\" --organization \"${ORG}\"" || \
            ...
            inform "Publish CCV ${ccv_id}"
                "/usr/bin/hammer content-view publish --id \"${ccv_id}\" --organization \"${ORG}\" --description \"Build ${BUILD_ID} of Job ${JOB_NAME} on ${JENKINS_URL}\"" || \
...
then
    ...
    do
        ccv_ver=$("/usr/bin/hammer --csv content-view version list --content-view-id ${ccv_id} --organization \"${ORG}\"" | awk -F',' '$1 ~ /^[0-9]+$/ {if ($3 > maxver) {maxver = $3; maxid = $1} } END {print maxid}')
        inform "Promoting version ${ccv_ver} of CCV ID ${ccv_id} to LCE ${TESTVM_ENV}"
        "/usr/bin/hammer content-view version promote --content-view-id \"${ccv_id}\" --organization \"${ORG}\" \
